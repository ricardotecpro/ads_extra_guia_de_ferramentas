...
def test_homepage_structure(page: Page, base_url):
    ...
    expect(heading).to_contain_text("Ferramentas para Projetos")
...
def test_lesson_01_page(page: Page, base_url):
    ...
    expect(page).to_have_title(re.compile(r"Aula 01 - Intro ao Ecossistema"))
    
    # Check main heading
    heading = page.locator("h1")
    expect(heading).to_contain_text("Ecossistema")
...
def test_lesson_16_page(page: Page, base_url):
    ...
    # Check title
    expect(page).to_have_title(re.compile(r"Aula 16 - Design e Figma"))
